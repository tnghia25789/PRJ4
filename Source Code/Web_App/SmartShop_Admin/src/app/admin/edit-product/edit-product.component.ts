...
export class EditProductComponent implements OnInit {
  ...
  update() {
    if(this.postForm.valid) {
      ...
      this.toastr.error('Failed To Update !', 'SYSTEM');
    }
  ...
  getProduct() {
    this.productService.getOne(this.id).subscribe(data => {
      ...
      this.postForm = new FormGroup({
        ...
        'price': new FormControl(this.product.price, [Validators.required, Validators.min(10)]),
  ...
  getCategories() {
    this.categoryService.getAll().subscribe(data => {
      ...
      this.toastr.error('Can Not Load Data ! Please Try Again.', 'SYSTEM');
    })
  }
