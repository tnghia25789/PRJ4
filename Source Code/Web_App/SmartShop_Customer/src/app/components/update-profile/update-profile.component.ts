...
export class UpdateProfileComponent implements OnInit {
  ...
  update() {
    if(this.postForm.valid) {
      ...
      this.customerService.update(this.customer.userId, this.customer).subscribe(data=>{
        this.toastr.success('Successful Updated !', 'SYSTEM');
        this.editFinish.emit('Done');
      })
    } else {
      this.toastr.error('Failed To Updated ! Please Try Again.', 'SYSTEM');
    }
    this.modalService.dismissAll();
  }

  //để lấy lên dữ liệu của customer rùi bind vô edit
  getCustomer() {
