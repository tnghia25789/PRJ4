...
export class CheckoutComponent implements OnInit {
    ...
    this.cartService.getCart(email).subscribe(data => {
      ...
      this.cartService.getAllDetail(this.cart.cartId).subscribe(data => {
        ...
        if (this.cartDetails.length == 0) {
          ...
          this.toastr.info('Please Add Some Products To Perform Payment !', 'SYSTEM');
        }
  ...
  checkOut() {
    if (this.postForm.valid) {
      ...
      this.toastr.error('Please Complete All Information !', 'SYSTEM');
    }
  }

  sendMessage(id:number) {
    let chatMessage = new ChatMessage(this.cart.user.name, ' placed an order');
    this.notificationService.post(new Notification(0, this.cart.user.name + ' placed an order ('+id+')')).subscribe(data => {
  ...
  private checkOutPaypal(): void {
    ...
    this.payPalConfig = {
        currency: 'USD',
        clientId: 'AQ4ZJJnU968Jsynv0WIPSZzugEh53i2zWhGGFQJcV02nwjtivelBgLxDM7LqmIKccvSJzlDlFL0k29pj',
        createOrderOnClient: (data) => < ICreateOrderRequest > {
            intent: 'CAPTURE',
            purchase_units: [{
                amount: {
                    currency_code: 'USD',
                    value:String(this.amount.toFixed(2)),
