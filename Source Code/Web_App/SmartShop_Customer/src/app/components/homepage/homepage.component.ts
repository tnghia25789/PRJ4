...
export class HomepageComponent implements OnInit {
  ...
  //lấy lên danh sách sản phẩm bán chạy nhất
  ...
  //lấy lên danh sách sản phẩm mới
  getAllProductLatest() {
  ...
  //lấy lên danh sách sản phẩm đc đánh giá cao
  getAllProductRated() {
      ...
      if (data == null) {
        this.customerService.getByEmail(email).subscribe(data => {
          ...
          this.favoriteService.post(new Favorites(0, new Customer(this.customer.userId), new Product(id))).subscribe(data => {
            this.toastr.success('Successful Added To Favorite List !', 'SYSTEM');
